...
function swm_footer() {
  ...
  $padding_top = get_option('swm_padding_top');
...
echo <<<EOT
	...
	jQuery(document).ready(function($) {
	  ...
	  if (winwidth >= $max) {
		...
		style.type = 'text/css';
		style.innerHTML = '.sticky-element-fixed { position: fixed; z-index: $z_index; top: ' + ptop + 'px; width: ' + elwidthpx + ' !important; }';
		...
		$(window).scroll(function() {
		  ...
		  if (winscroll + ptop + elheight > endtop) {
...
if (!is_admin()){
	...
	function swm_register_mysettings() {
		...
		register_setting( 'swm-settings-group', 'swm_z_index', 'swm_validate_optional' );
		register_setting( 'swm-settings-group', 'swm_padding_top', 'swm_validate_optional' );
	}
	...
	function swm_validate_optional($input) {
	  if ($input == '') return '0';
	  return $input;
	}
	function swm_admin() {
		...
		if($tab == 'first' ) {?>
			<div class="wrap">
			<h2 style="float: left;">Sticky Element</h2><br /><br /><br />
			<em style="float: left;">Note: Fields with asterisk (<pre style="display: inline-block; color: red;">*</pre>) are required! </em>
			<form method="post" action="options.php">
			<?php settings_fields( 'swm-settings-group' ); ?>
				<table class="form-table">
					
					<tr valign="top">
					<th scope="row"><pre style="display: inline-block; color: red;">*</pre> Element you want to make sticky</th>
					<td><input type="text" name="swm" value="<?php echo get_option('swm'); ?>" placeholder="e.g. #sidebar" required/></td>
					</tr>
					 
					<tr valign="top">
					<th scope="row"><pre style="display: inline-block; color: red;">*</pre> Element that pushes sticky upward</th>
					<td><input type="text" name="swm_end" value="<?php echo get_option('swm_end'); ?>" placeholder="e.g. #footer" required/></td>
					</tr>
					
					<tr valign="top">
					<th scope="row"><pre style="display: inline-block; color: red;">*</pre> Active when window is wider than</th>
					<td><input type="number" min="0" name="swm_max" value="<?php echo get_option('swm_max'); ?>" placeholder="e.g. 960" required/>pixels</td>
					</tr>
					
					<tr valign="top">
					<th scope="row">Z-index (optional)</th>
					<td><input type="number" min="0" name="swm_z_index" value="<?php echo get_option('swm_z_index'); ?>" placeholder="e.g. 20" /></td>
					</tr>
					
					<tr valign="top">
					<th scope="row">Space between top of page and sticky element (optional)</th>
					<td><input type="number" min="0" default="0" name="swm_padding_top" value="<?php echo get_option('swm_padding_top'); ?>" placeholder="e.g. 20" />pixels</td>
					</tr>
				</table>
